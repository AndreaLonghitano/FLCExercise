...
exp:  SUM IDENTIFIER  IDENTIFIER {
	if(getVariable(program,$2)->isArray || getVariable(program,$3)->isArray) notifyError(AXE_INVALID_TYPE);

	$1.reg_sum=gen_load_immediate(program,0);

	}
	OUT OF IDENTIFIER {
	  if(!getVariable(program,$7)->isArray) notifyError(AXE_INVALID_TYPE);
	  $1.reg_counter=gen_load_immediate(program,0);
	  $1.reg_counter2=gen_load_immediate(program,1);
	
	  $1.label_end=newLabel(program);
	  $1.label_cond=newLabel(program);
	  assignLabel(program,$1.label_cond);
	  gen_subi_instruction(program,getNewRegister(program),$1.reg_counter2,getVariable(program,$7)->arraySize);
	  gen_beq_instruction(program,$1.label_end);
	  int i=get_symbol_location(program,$2);
	  int j=get_symbol_location(program,$3);
	  int reg1=loadArrayElement(program,$7,create_expression($1.reg_counter,REGISTER));
	  gen_add_instruction(program,i,REG_0,reg1,CG_DIRECT_ALL);
	  int reg2=loadArrayElement(program,$7,create_expression($1.reg_counter2,REGISTER));
	  gen_add_instruction(program,i,REG_0,reg2,CG_DIRECT_ALL);
	}
	AS exp {
	 int reg=getNewRegister(program);
         if($10.expression_type==IMMEDIATE)  gen_addi_instruction(program,$1.reg_sum,$1.reg_sum,$10.value);
	 else  gen_add_instruction(program,$1.reg_sum,$1.reg_sum,$10.value,CG_DIRECT_ALL);
	 gen_addi_instruction(program,$1.reg_counter,$1.reg_counter,1);
	 gen_addi_instruction(program,$1.reg_counter2,$1.reg_counter2,1);
	 gen_bt_instruction(program,$1.label_cond,0);

	 assignLabel(program,$1.label_end);
	 $$=create_expression($1.reg_sum,REGISTER);
